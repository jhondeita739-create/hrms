...
import { ApplicantProfileCompletionForm } from "@/components/applicant-profile-completion-form";
...
export default async function TrackApplicationPage({
  ...
  const candidateToken = String((await searchParams).token || "");
  const token = /^[a-f0-9]{64}$/i.test(candidateToken) ? candidateToken : "";
  ...
  return (
    <main className="relative min-h-[calc(100vh-80px)] overflow-hidden bg-[#FAFBFF] selection:bg-brand-200">
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-4 py-12 sm:px-6 lg:px-8 lg:py-20">
        ...
        {token ? (
          <ApplicantProfileCompletionForm token={token} />
        ) : (
          <div className="grid gap-12 lg:grid-cols-[1fr_1.2fr] lg:items-center">
            <div>
              ...
              <p className="mt-6 text-lg leading-relaxed text-slate-500">
                Enter the reference from your confirmation and the email used to apply. Interview qualification and profile requests appear here.
              </p>
            </div>
